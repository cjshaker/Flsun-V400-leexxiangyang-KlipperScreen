...
class KlippyGtk:
    ...
    #zzcatvs 2023/06/26
    def Dialog_button(self, screen, content,types=None):
        dialog = Gtk.Dialog()
        #if types == "reset":
        dialog.set_default_size(screen.width, screen.height)
        ...
        content_area.set_margin_start(15)
        content_area.set_margin_end(15)
        content_area.set_margin_top(280)
        content_area.set_margin_bottom(15)
        content_area.add(content)
        dialog.show_all()
        dialog.get_window().set_cursor(Gdk.Cursor(Gdk.CursorType.BLANK_CURSOR))
        ...
        return dialog
     #2023.7.17
    def Dialog_buttondd(self, screen, content):
        dialog = Gtk.Dialog()
        dialog.set_resizable(False)
        dialog.set_transient_for(screen)
        dialog.set_modal(True)
        dialog.get_style_context().add_class("dialogg")
    ...
    def menu_back(self):
        os.system("cp -a /home/pi/filebak/KlipperScreen/warning.txt /home/pi/KlipperScreen/warning.txt")
    
    def adxl345_back(self):
        os.system("bash /home/pi/KlipperScreen/scripts/printer/adxl345.sh")
