...
class SettingsPanel(ScreenPanel):
    def initialize(self, panel_name):
        ...
        options.append({"reset": {
            "name": _("Reset Printer System"),
            "type": "resetpad",
            "resetpad": "reset_pad"
        }})


        citys = ["GMT+0:00", "GMT+1:00", "GMT+2:00", "GMT+3:00", "GMT+4:00", "GMT+5:00", "GMT+6:00", "GMT+7:00", "GMT+8:00", "GMT+9:00", "GMT+10:00", "GMT+11:00", "GMT+12:00",  "GMT-1:00", "GMT-2:00", "GMT-3:00", "GMT-4:00", "GMT-5:00", "GMT-6:00", "GMT-7:00", "GMT-8:00", "GMT-9:00", "GMT-10:00", "GMT-11:00", "GMT-12:00"]
        ...
        for option in options:
            ...
            if name == "time_zone":
                for city in citys:
                    option[name]['options'].append({"name": _(city), "value": city})
    ...
    def add_option(self, boxname, opt_array, opt_name, option):
        ...
        elif option['type'] == "resetpad":
            open = self._gtk.ButtonImage("settings", None, "color3")
            open.connect("clicked", self.load_reset, option['resetpad'])
            open.set_hexpand(False)
            open.set_halign(Gtk.Align.END)
            dev.add(open)
    ...
    def load_reset(self, widget, option):
        _ = self.lang.gettext
        if option == "reset_pad":
            buttons = [{"name": _("Accept"), "response": Gtk.ResponseType.OK}, {"name": _("Cancel"), "response": Gtk.ResponseType.CANCEL}]
            label = Gtk.Label()
            label.set_markup(_("Are you sure reset Printer System?\nIf you click 'Accept', the system will return to the factory state."))
    ...
    def on_dropdown_change(self, combo, section, option, callback=None):
        ...
        if tree_iter is not None:
            ...
            if option == "time_zone": 
                ...
                label = Gtk.Label()
                label.set_markup(_("Are you sure switch time zone to %s\n If you click 'accept', the system will automatically restart") % (value))
                label.set_hexpand(True)
                label.set_halign(Gtk.Align.CENTER)
                label.set_vexpand(True)
                label.set_valign(Gtk.Align.CENTER)
                ...
                dialog = self._gtk.Dialog(self._screen, buttons, label, self.switch_time_zone, value)
                
    def switch_time_zone(self, widget, response_id, value):
        if response_id == Gtk.ResponseType.OK:
            logging.debug("switch time zone to %s" % value)
            if '-' in value:
                os.system("echo " + "Etc/GMT+" + value[4:-3] + " > /home/pi/time_zone1 && reboot")
            if '+' in value:
                os.system("echo " + "Etc/GMT-" + value[4:-3] + " > /home/pi/time_zone1 && reboot")
        widget.destroy()
