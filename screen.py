...
from ks_includes.widgets.keyboard import Keyboard #flsun add
...
class KlipperScreen(Gtk.Window):
    ...
    def _load_panel(self, panel, *args):
        if panel not in self.load_panel:
            ...
            os.system("sh /home/pi/mjpg-streamer/scripts/make.sh")
    ...
    def show_panel(self, panel_name, type, title, remove=None, pop=True, **kwargs):
        ...
        try:
            ...
            # elif remove == 0: #zzcatvs add 2023-7-5
            ...
            if panel_name == 'guide': #flsun add 
                self._cur_panels.append('splash_screen')
        ...
        if len(self._cur_panels) > 0:
            ...
            if pop is True:
                ...
                if len(self._cur_panels) > 0:
                    ...
                    #self.base_panel.show_back(False if len(self._cur_panels) == 1 else self._screen._ws.klippy.emergency_stop()) #zzcatvs add
    ...
    def _menu_go_back(self, widget=None):
        ...
        with open("warning.txt") as file_object:
            word = file_object.read()
            if "autolevel2" in word:
                self.gtk.menu_back()
                self.printer_ready()
            elif "adxl3451" in word:
                self.gtk.menu_back()
                self.printer_ready() 
            else:    
        ...
        with open("warning.txt") as file_object:
            for line in file_object:
                if count == 1:
                    word = line.rstrip()
                count += 1
        if "Guide_True" in word:
            self.show_panel('boot_language', 'boot_language', 'boot_language', 2)
            os.system("service klipper-1 stop")
        else:
    ...
    def show_keyboard(self, widget=None, entry=None): #flsun modify ,for keyboard
